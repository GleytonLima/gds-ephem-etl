...
class BuscarRecomendacaoTecnicaEphemWorkTest {

    @Mock
    private RecomendacaoTecnicaRepository repository;
    ...
    @InjectMocks
    private BuscarRecomendacaoTecnicaEphemWork work;
    ...
    void testProcessarWithApiResponse() {
        ...
        when(restTemplate.getForObject("http://example.com/api-integracao/v1/models/eoc.signal.technical.recommendation?page=0&size=100", ModelApiResponse.class))
                .thenReturn(ModelApiResponse);

        work.processar();

        verify(repository).deleteAll();
        verify(repository).save(any(RecomendacaoTecnica.class));
    }
    ...
    void testProcessarWithException() {
        ...
        verify(repository, never()).save(any(RecomendacaoTecnica.class));
    }
}