...
class BuscarFontesEphemWorkTest {
    ...
    @InjectMocks
    private BuscarFontesEphemWork work;
    ...
    void testProcessarWithApiResponse() {
        ...
        when(restTemplate.getForObject("http://example.com/api-integracao/v1/models/eoc.signal.sources?page=0&size=100", ModelApiResponse.class))
                .thenReturn(ModelApiResponse);

        work.processar();

        verify(repository).deleteAll();
        verify(repository).save(any(Fonte.class));
    }
    ...
    void testProcessarWithException() {
        ...
        verify(repository, never()).save(any(Fonte.class));
    }
}